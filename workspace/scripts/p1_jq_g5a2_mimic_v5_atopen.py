...
LOGGER = logging.getLogger("p1_jq_mimic_v5")
...
OUTPUT_DIR = PROJECT_ROOT / "workspace/research/alpha_mining/p1_jq_g5a2_mimic_v5_atopen_run"
...
AT_OPEN_FILTER = True
TOP_K_CANDIDATES = 2 * STOCK_NUM  # 24, matches JQ's 2*g.stock_num buffer
...
class JoinQuantG5MimicStrategy(Strategy):
    ...
    def before_market_open(self, context: BacktestContext) -> list[Order]:
        ...
        # 3. Normal schedule rebalance (only on weekly-Tuesday rebal dates)
...
def compute_rebalance_schedule(
    ...
    # v5: load BOTH the size factor AND today's $open + $up_limit (no Ref —
    # these are simultaneous with the 9:30 trading moment so using them at
    # decision-time is PIT-correct, not lookahead).
    feature_list = ["Ref($total_mv, 1)", "$open", "$up_limit"]
    LOGGER.info(
    ...
    df.columns = ["total_mv_lag1", "today_open", "today_up_limit"]
    LOGGER.info(f"Feature loaded: {df.shape}")
    # MultiIndex (instrument, datetime) per Qlib convention
    df = df.dropna(subset=["total_mv_lag1"])  # require size; allow $open/$up_limit NaN

    # For each rebal date, take top-N smallest from eligible set
    schedule: dict[pd.Timestamp, dict[str, float]] = {}
    n_locked_skipped = 0
    n_short_picks = 0
    for rd in rebal_dates:
        ...
        if AT_OPEN_FILTER:
            # v5: take top-K candidates by size, then filter out those locked
            # at today's open (today_open >= today_up_limit), then take first N.
            top_k = slice_.nsmallest(TOP_K_CANDIDATES, "total_mv_lag1")
            # Lock condition: today's open price is at or above today's up_limit.
            # Use small tolerance for floating-point equality.
            tol = 1e-4
            unlocked_mask = (
                top_k["today_open"].notna()
                & top_k["today_up_limit"].notna()
                & (top_k["today_open"] < top_k["today_up_limit"] - tol)
            )
            picks_q = top_k[unlocked_mask].head(stock_num)
            n_skipped_this_date = (~unlocked_mask).sum()
            n_locked_skipped += int(n_skipped_this_date)
            if len(picks_q) < stock_num:
                n_short_picks += 1
            smallest = picks_q
        else:
            smallest = slice_.nsmallest(stock_num, "total_mv_lag1")

        weights = {idx.upper().replace("_", "."): 1.0 / stock_num for idx in smallest.index}
        if weights:
            schedule[rd] = weights

    LOGGER.info(
        f"Schedule built: {len(schedule)} rebalance dates. "
        f"At-open filter skipped {n_locked_skipped} locked candidates total. "
        f"{n_short_picks} rebal dates had < {stock_num} non-locked picks."
    )
    return schedule
