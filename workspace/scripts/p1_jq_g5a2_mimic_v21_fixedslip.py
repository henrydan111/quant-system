...
LOGGER = logging.getLogger("p1_jq_mimic_v21")
...
SLIPPAGE_YUAN = 3.0 / 10000.0  # v21: 0.0003 ¥/share — TRUE match to JQ FixedSlippage(3/10000)
                               # (v8-v19 wrongly used PctSlippage(0.0003)=3bps, ~10x larger)
...
OUTPUT_DIR = PROJECT_ROOT / "workspace/research/alpha_mining/p1_jq_g5a2_mimic_v21_fixedslip_run"
...
class JoinQuantG5MimicStrategyV6(Strategy):
    ...
    def before_market_open(self, context: BacktestContext) -> list[Order]:
        ...
        if n_empty > 0:
            # v19 FIX: guard each prev-price term against NaN / non-positive.
            # A suspended sold position has a NaN prev-close; without this guard
            # the NaN poisons est_sell_proceeds → avail_cash → value_per_new,
            # the `value_per_new > 1.0` test is False, and NO buys are generated —
            # which left the portfolio stuck in cash for ~18 days after the
            # 2015-08-19 股灾 stoploss (the dominant source of the local-vs-JQ gap).
            est_sell_proceeds = 0.0
            for c in sold_codes:
                pos = current_positions.get(c)
                if pos is None:
                    continue
                px = prev_prices.get(c, None)
                if px is None or not np.isfinite(px) or px <= 0:
                    px = float(pos.avg_cost) if (pos.avg_cost and np.isfinite(pos.avg_cost) and pos.avg_cost > 0) else 0.0
                if np.isfinite(px) and px > 0:
                    est_sell_proceeds += pos.shares * px
            avail_cash = context.portfolio.cash + est_sell_proceeds
            if not np.isfinite(avail_cash):
                avail_cash = context.portfolio.cash
            value_per_new = avail_cash / n_empty
            if np.isfinite(value_per_new) and value_per_new > 1.0:
        ...
        if yhl:
            ...
            if yhl_sold_codes and self.g.last_ranked_list:
                ...
                if slots_to_fill > 0:
                    ...
                    # v18: JQ check_remain_amount sizing — available cash / empty slots.
                    # v19 FIX: guard close prices against NaN (suspended) before summing.
                    est_proceeds = 0.0
...
def main() -> int:
    ...
    result = backtester.run(
        ...
        slippage=FixedSlippage(SLIPPAGE_YUAN),
