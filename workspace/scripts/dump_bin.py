...
class DumpDataBase:
    ...
    def _should_use_process_pool(self) -> bool:
        return bool(self.works and int(self.works) > 1)
        ...
        with tqdm(total=len(self.df_files)) as p_bar:
            if self._should_use_process_pool():
                with ProcessPoolExecutor(max_workers=self.works) as executor:
                    iterator = zip(self.df_files, executor.map(_fun, self.df_files))
                    for file_path, ((_begin_time, _end_time), _set_calendars) in iterator:
        ...
        with tqdm(total=len(new_stock_files)) as p_bar:
            if self._should_use_process_pool():
                with ProcessPoolExecutor(max_workers=self.works) as execute:
                    iterator = zip(new_stock_files, execute.map(_fun, new_stock_files))
                    for file_path, (_begin_time, _end_time) in iterator:
                        if isinstance(_begin_time, pd.Timestamp) and isinstance(_end_time, pd.Timestamp):
                ...
                for file_path in new_stock_files:
                    _begin_time, _end_time = _fun(file_path)
                    if isinstance(_begin_time, pd.Timestamp) and isinstance(_end_time, pd.Timestamp):
                        symbol = self.get_symbol_from_file(file_path).upper()
                        _dt_map = self._old_instruments.setdefault(symbol, dict())
                        _dt_map[self.INSTRUMENTS_START_FIELD] = self._format_datetime(_begin_time)
        ...
        for _code, _df in self._all_data.groupby(self.symbol_field_name, group_keys=False):
            ...
            if _code in self._update_instruments:
                ...
                if _update_calendars:
                    ...
                    grouped_data.append((_code, _df, _update_calendars))
            else:
                # new stock
                ...
                grouped_data.append((_code, _df, self._new_calendar_list))

        with tqdm(total=len(grouped_data)) as p_bar:
            if self._should_use_process_pool():
                with ProcessPoolExecutor(max_workers=self.works) as executor:
                    futures = {
                        executor.submit(self._dump_bin, grouped_df, calendars): code
                        for code, grouped_df, calendars in grouped_data
                    }
                ...
                for code, grouped_df, calendars in grouped_data:
                    try:
                        self._dump_bin(grouped_df, calendars)
                    except Exception:
                        error_code[code] = traceback.format_exc()
                    p_bar.update()
