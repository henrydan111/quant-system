...
LOGGER = logging.getLogger("p1_jq_mimic_v8")
...
OUTPUT_DIR = PROJECT_ROOT / "workspace/research/alpha_mining/p1_jq_g5a2_mimic_v8_100k_capital_run"
...
class JoinQuantG5MimicStrategyV6(Strategy):
    ...
    def _check_market_stoploss(self, context: BacktestContext) -> bool:
        ...
        if ratio <= self.stoploss_market:
            LOGGER.info(
                "Market stoploss fired on %s: mean(close/open)=%.4f over %d (threshold %.2f)",
    ...
    def before_market_open(self, context: BacktestContext) -> list[Order]:
        ...
        if self._check_market_stoploss(context):
            ...
            self.g.yesterday_HL_set = set()
            return self._sell_all(context, "market_stoploss")
        ...
        for code in target_unlocked:
            ...
            if buy_value > 1.0:
                orders.append(Order(
                    ...
                    target_value=buy_value, reason="rebalance_buy",
                ))
        return orders
...
def compute_rebalance_schedule_v6(
    ...
    feature_list = ["Ref($total_mv, 1)", "$open", "$close", "$up_limit"]
    LOGGER.info(
    ...
    df.columns = ["total_mv_lag1", "today_open", "today_close", "today_up_limit"]
