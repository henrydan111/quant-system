...
LOGGER = logging.getLogger("p1_jq_mimic_v20")
...
OUTPUT_DIR = PROJECT_ROOT / "workspace/research/alpha_mining/p1_jq_g5a2_mimic_v20_v11_nanfix_run"
...
class JoinQuantG5MimicStrategyV6(Strategy):
    ...
    def before_market_open(self, context: BacktestContext) -> list[Order]:
        ...
        # v20 FIX: NaN-safe portfolio valuation. A held position that is suspended
        # has a NaN prev-close; passing it to total_value() returns NaN, which
        # poisons per_slot_value → buy_value (max(NaN-…)) → `buy_value > 1.0` is
        # False → NO buys generated → the portfolio gets stuck in cash after a
        # stoploss/pass-month clearout (the dominant local-vs-JoinQuant gap, e.g.
        # the entire 2015-08-19→09-14 股灾 recovery was missed). Build a clean
        # price dict (finite, positive) and value missing names at avg_cost.
        safe_prices = {k: v for k, v in prev_prices.items()
                       if v is not None and np.isfinite(v) and v > 0}
        portfolio_value = context.portfolio.cash
        for _code, _pos in context.portfolio.positions.items():
            _px = safe_prices.get(_code)
            if _px is None or not np.isfinite(_px) or _px <= 0:
                _px = float(_pos.avg_cost) if (_pos.avg_cost and np.isfinite(_pos.avg_cost) and _pos.avg_cost > 0) else 0.0
            portfolio_value += _pos.shares * _px
        ...
        # TRIM positions that are in target but oversized
        for code in sorted(held & target_set):
            ...
            diff_value = current_value - per_slot_value
            ...
            if shares_to_sell > 0:
                orders.append(Order(
                    ...
                    target_shares=shares_to_sell, reason="rebalance_trim",
                ))

        # BUY new + top-up under-sized positions for target stocks
        for code in target_unlocked:
            pos = current_positions.get(code)
            ref_price = float(prev_prices.get(code, pos.avg_cost if pos else 0))
            # v20 FIX: a suspended target stock has NaN prev-close → NaN current_value
            # → NaN buy_value → this stock's buy is silently skipped. Guard it so a
            # held-but-suspended target name doesn't lose its top-up slot.
            if not np.isfinite(ref_price) or ref_price <= 0:
                current_value = 0.0
            else:
                current_value = 0.0 if pos is None else pos.shares * ref_price
            buy_value = max(per_slot_value - current_value, 0.0)
            if np.isfinite(buy_value) and buy_value > 1.0:
                orders.append(Order(
                    code=code, direction="buy",
                    target_value=buy_value, reason="rebalance_buy" if not stoploss_fired else "stoploss_then_rebal_buy",
                ))
        ...
        if yhl:
            ...
            if yhl_sold_codes and self.g.last_ranked_list:
                ...
                if slots_to_fill > 0:
                    ...
                    # Compute per-slot value at today's close
                    today_close_prices = {}
                    for c in sub_codes + list(held):
                        q = self._get_quote(c, context.date)
                        if q is not None and q.get("close") is not None:
                            today_close_prices[c] = q["close"]
                    portfolio_value = context.portfolio.total_value(today_close_prices)
                    if portfolio_value <= 0:
                        portfolio_value = context.portfolio.cash
                    per_slot_value = portfolio_value / max(self.stock_num, 1)
                    for code in sub_codes:
                        if per_slot_value > 1.0:
                            orders.append(Order(
                                code=code, direction="buy",
