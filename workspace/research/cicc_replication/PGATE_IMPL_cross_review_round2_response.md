...
| **1 / Cond-1** 7-domain matrix PRODUCER not a DAG prerequisite | **RESOLVED via GPT option (b)** | The gate already fail-closes without matrix evidence (`availability_audit_missing` → `evidence_only`, surfaced distinctly as `refused_by_missing_prerequisite`), and the producer/generate-command EXISTS (the F2 matrix drivers; `import_matrix_evidence.py` writes freshness-correct `source_hash`). Demonstrated end-to-end 2026-06-14: `comp_cicc_profit` `register → produce 7-domain matrix → import → re-gate` advanced `evidence_only → candidate_ceiling` (`coverage_pass` acquired; still capped by `short_oos_power_floor_fail`; still not promoted). The documented operational flow is the prerequisite. The heavier **option (a)** — auto-wiring the matrix step into the lifecycle DAG — remains an OPTIONAL future convenience, not a blocker (a controlled, human-gated cadence runs the producer per batch explicitly). |
...
## Status after the 2026-06-14 follow-up: the gate is operationally complete for a controlled univ_all wave

Finding-1 is **resolved via option (b)** (above) — the matrix producer ran, the import is freshness-bound, and the prerequisite-clearing flow is demonstrated. With every R2 condition now either folded in or satisfied:

- **Cleared:** a controlled, human-gated **univ_all** CICC publish-to-candidate cadence — the operator runs the matrix producer for the batch (`unified_eval_matrix_layer2.py` for composites / `unified_eval_universe_matrix.py` for base) → `import_matrix_evidence.py --live` → the gate adjudicates (fail-closing any factor whose evidence wasn't produced).
- **Still deferred (scope, not the univ_all wave):** Finding-1 option (a) auto-DAG-wiring (convenience) · `require_replication_governance` run-flag scoping · adjudication-error audit row · `approximate=False` enforcement at the **sealed-OOS** gate (the candidate gate persists the flag; `comp_cicc_profit`'s quarantine is `2023-02-09` **approximate=True** — must be recomputed with an injected trade calendar before any OOS spend) · F3 stamp / F9-full declared-domain / F11 linkage ledger.

Field registration (D4a) and offline P-OP can proceed in parallel.
