...
| **C1** | consumption module [news_session_embed.py](engine/news_session_embed.py) ONLY (`consume_news_decision`: committed-evidence artifact rebuild → THE decision-level canonical door → seat in `run_seat` shape with recomputed final + identity block, `opaque_scalar` on the scalar branch / `opaque_external` on vector_only; `NothingToDecide` → `no_decision` fallback signal; everything else fail-closed error seat). **NO `analyst_chain` change ships in C1** (review round-1 P1#1: the manifest hashes that file's bytes into the frozen v3.1 contract — even a default-OFF hook parameter collides; the file is byte-hash-pinned by a guard test until the bump). ALL session wiring (hook, news-seat branch, `nf_decision` archive embedding, opaque-scalar judge semantics, full-timestamp cutoff binding) = the FROZEN WIRING OBLIGATIONS in the module docstring, discharged by the final-integration chain-version bump. Implemented 2026-07-24, pending Tier-2 close-out. Design: [NF_UNIT_C1_DESIGN.md](NF_UNIT_C1_DESIGN.md) | build | 0 | P4 |
