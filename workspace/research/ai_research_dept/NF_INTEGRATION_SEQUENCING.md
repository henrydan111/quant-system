...
| **P4** | per-stock: `record_decision` → `execute_news_decision` (factor + penalty legs) → `seal_decision_archive`. **Carries the FROZEN P4 OBLIGATION** (GPT P3b round-1 P1): require + bind the `AssemblyProvenance`, write `assembly_hash` into the ledger, embed the payload in the archive under a bumped `_ARCHIVE_SCHEMA` v2, refusal tests. Until this lands the upstream chain is provable only in memory — the sealed archive cannot say which P2/P3a run, stock, or facts produced the artifact. Spec: [NF_UNIT_P3B_DESIGN.md](NF_UNIT_P3B_DESIGN.md) | wire | 2 | P3 |
