...
| **P2** | **MARKET-WIDE** (corrected from "per-stock"): text_store read → `build_cluster_snapshots` → `route_cluster` (existing router) → `assess_flash` (with P1 typing) → sealed assessed-flash artifact. Per-stock selection is P3, since routing (not clustering) associates a flash to stocks. Design frozen: NF_UNIT_P2_DESIGN.md | wire (+ new read path) | 0 | P1 |
