...
| **P4b** | per-stock decision driver [news_flash_decide.py](engine/news_flash_decide.py): deterministic `nf:{class}:{ts_code}:{cutoff}` identity → committed-evidence assemble → record → execute (2 LLM legs) → seal, with crash-safe idempotent re-entry (success never re-executed; commit-but-no-seal recovers pure-disk; hard_failed retries fresh). Identities-only return. Tier-2, pending review. | wire | 2 | P4a |
