...
| **C1** | **DONE + SOUND 2026-07-24** (Tier-2, closed at `12917ee` after a 4-round arc: round-1 2×P1 [default-OFF hook still changed the frozen v3.1 CONTRACT — the manifest hashes `analyst_chain.py`'s bytes; the legacy judge zeroed the opaque sealed score] → unit SHRANK to the consumption module only, `analyst_chain.py` byte-reverted; round-2 3×P2 [scalar-only `opaque_scalar` flag discipline, stale-wiring doc sweep, byte→LF-canonical pin]; round-3 2×P2 at budget → user arbitration → fold [Unit-2 spec marked HISTORICAL; LF-normalized portable pin]; round-4 confirmation SOUND). Ships [news_session_embed.py](engine/news_session_embed.py) ONLY: `consume_news_decision` (committed-evidence rebuild → decision-level canonical door → `run_seat`-shaped seat with recomputed final + identity block; `opaque_external` on every consumed seat, `opaque_scalar` on scalar only; `no_decision` fallback signal; fail-closed error seat otherwise). ALL session wiring = the FROZEN WIRING OBLIGATIONS (bump-first / opaque-scalar judge semantics / full-timestamp cutoff / fixed fallback dichotomy), discharged by the final-integration chain-version bump; `analyst_chain.py` LF-canonical-hash-pinned until then. | build | 0 | P4 |

**WAVE STATUS 2026-07-24: every NF unit CLOSED SOUND** (P1 → P2 → P3a → P3b → P4a → P4b → C1).
Remaining = the final-integration **chain-version-bump unit**: discharge the four frozen wiring
obligations, bind `NewsScoringContract` from the on-disk `ChainContract`, bind the production
...
| **BUMP** | **DONE + SOUND 2026-07-24** (Tier-2, closed at `540b510` after a 3-round arc, zero declines: round-1 3xP1 [free callback bypassed binding -> roots-only `nf_roots` + engine chokepoint `_consume_nf_seat`; nf_contract values not locked -> exact value lock vs the frozen constant, proven on a real on-disk manifest fixture; executed consumer outside the engine contract -> `news_session_embed.py` joins the hash]; round-2 P1+P2 [cache bypassed the chokepoint -> sealed `nf_mode` in the archive identity + cross-mode reuse fails closed, never overwrites; roots preflight hoisted before cache/any seat]; round-3 SOUND + post-verdict stale-phrasing sweep [nf_news->nf_roots]) — — the four frozen wiring obligations DISCHARGED: (a) roots-only NF parameter (`nf_roots`, five trusted dirs; the free-form `nf_news` callable was retired by review round-1 P1#1 - consumption/cutoff/contract/identity all engine-derived in `_consume_nf_seat`; `nf_mode` sealed into the archive identity, cross-mode cache reuse fails closed) + news-seat branch + strictly-additive `nf_decision` identity block land WITH the version bump (byte pin moved in the same commit; hook default OFF — enablement stays the caller's/FORWARD_PREREG governed runner's); (b) `judge()` passes `opaque_scalar` seats through (`adj_final == final`; empty-list recompute that zeroed the sealed 49.0 is dead; legacy discount path untouched); (c) manifest gains the frozen `nf_contract` section (`c16_news_horizon_v1` / `primary_horizon` / **1-3d** / **18:00:00** / `forward`, user-frozen 2026-07-24) + `nf_contract_from_chain` / `nf_cutoff_for_day` (bare dates cannot reach the NF doors); (d) fallback dichotomy preserved in the hook branch. Platform `RENDER_VERSION` synced. Design: [NF_UNIT_BUMP_DESIGN.md](NF_UNIT_BUMP_DESIGN.md) | wire | 0 | C1 |
