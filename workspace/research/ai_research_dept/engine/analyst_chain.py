...
CHAIN_VERSION = "chain_v2.1"  # v2.1: GPT REVISE 修复 —— 档案版本隔离(版本目录+匹配才跳过+
#   manifest)/scorecard 严格校验(重名拒收+注册维恰一次+分数域+证伪schema)/空头 typed 校验
#   +falsifier_id 快径/行ID 机械接地+ID独占+间接≤3钳位+披露围栏/幸存者修复/单位修复;
#   v2.0: 审计 v1 输入升级;v1.3: news并概念通道;v1.2: +market_context;v1.1: +业务构成
...
def _normalize_falsifiers(wcw) -> list:
    """保留合格 {condition, observable_in} 条目,裹裸 dict,截断 5。"""
...
def run_seat(seat: str, prompt: str, payload: dict, card_text: str,
    ...
    rec["what_could_weaken"] = _normalize_falsifiers(rec.get("what_could_weaken"))
    ...
    return {"final": final, "record": rec, "scored_dims": n_scored, "total_dims": n_dims,
            "fence_stats": fence_stats, "model": r.model, "usage": r.usage}
...
def run_bear(cards: dict, seat_results: dict, falsifiers: dict,
    ...
    fid_set = {f["falsifier_id"] for fl in falsifiers.values() for f in fl}
    return validate_bear_record(rec, all_cards, SEAT_WEIGHTS, fid_set)
...
def run_stock(code: str, day: str, facts: pd.DataFrame, pv: pd.DataFrame,
    ...
    if arch_path.exists():
        # 版本隔离(GPT Blocker-1):只有 chain_version 完全一致才复用;否则重算
    ...
    if ok_seats:
        try:
            bear = run_bear(cards, ok_seats, falsifiers, audit)
...
def main() -> int:
    ...
    # 版本目录 + manifest(GPT Blocker-1:档案按链版本隔离,输入指纹落盘)
    vdir = CHAIN_DIR / CHAIN_VERSION
    vdir.mkdir(parents=True, exist_ok=True)
    (vdir / "manifest.json").write_text(json.dumps(
        build_manifest(pv, retr, regime), ensure_ascii=False, indent=1), encoding="utf-8")
