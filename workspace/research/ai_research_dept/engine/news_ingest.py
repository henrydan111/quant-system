...
importance=事件对该标的基本面/交易的重大性(5=重大合同/业绩拐点,0=无关噪音)。"""

#: 宏观批附加(review M4:macro 批必须显式请求 macro_type;re-review Major-5:
#  分型优先级与 news_taxonomy.MACRO_TYPE_DIMENSION 派生表配套,两处必须一致)
_MACRO_TYPE_APPENDIX = """
【宏观批附加】每条另出 "macro_type":"货币政策|财政政策|监管全局|地缘外围|大盘资金面|商品汇率|行业景气|external_shock"。
归类优先级(必须遵守):离散外部冲击(外盘利率决议/商品价格剧变/汇率急变/地缘突发事件)
一律归 external_shock,优先于 地缘外围/商品汇率/货币政策;国内货币/财政/监管动作归各自
政策类;非冲击性的外围情绪、常规商品汇率波动才归 地缘外围/商品汇率。
无法归类则省略该字段(下游标 not_applicable)。"""
...
def type_batch(items: list[dict], call_fn, *, macro: bool = False) -> list[dict]:
    ...
    for i in requested:                           # 按输入序
        ...
        # importance_0_5(§2 step 6):字面 int ∈ [0,5] 才收;否则保守 2(不触发 D7
        # ≥4 拆行门、不排前列;bool 排除——True 不是 1)
        raw_imp = r.get("importance")
        importance = raw_imp if (type(raw_imp) is int and 0 <= raw_imp <= 5) else 2
        rec_i = {
            "idx": i,
