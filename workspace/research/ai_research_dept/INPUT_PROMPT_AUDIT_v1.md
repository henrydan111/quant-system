...
1. **近 8 季核心序列节**(P1;fact_table 已为 10 年分位加载了季度史,渲染即可,无新加载):
   ```
   ◆ 关键指标近8季(旧→新)
...
2. **P0 间接节类型配额 + 聚合行**:每 event_type 最多 3 行明细;同类型溢出的压缩成一条聚合行
   (聚合是**更强**的信号形态,不是丢信息):
   `- [概念聚合]业绩预告:概念同伴30日内共31条(利好9/利空19/中性3)`
   注意:**这只动渲染层**,检索 parquet/配置/快照不变(§6.5)。
...
### 6.5 层边界声明(为什么这不触发检索配置变更纪律)

检索层(倒排/通道权重/衰减/精筛/快照 id)**一概不动**;本提案改的是:
① regime/fact/pv 三个**确定性 builder** 的输出丰度;② news_card 的**渲染层**(对既有检索
结果的呈现:排序、配额、聚合、时间标注)。C16b 管的是检索配置候选;卡渲染变更走 chain 版本
纪律(chain_v2.0)。全部为重放期变更,NON_EVIDENTIARY;首轮前向前会按 FORWARD_RETRIEVAL_PREREG
连同卡版本一起冻结。
...
(pit loader 已加载面板、provider bins、事件库);无新 D.features 裸调;
Tushare 零新抓取(share_float 仅立项)。✔
**PIT**:§6.3 逐项表;唯一披露语义微妙项是两融 T+1,已要求渲染标注"截至 D-1"。✔
**C16 遏制**:LLM 输出 schema 零变化;裁判零变化;所有新增只在输入端。✔
**C15**:卡内容变化不改 payload=数据的地位。✔
**多重检验/OOS**:不适用(无评分口径变更用于选股;composite 仍 research_summary 禁入选股)。✔
**无对冲措辞**:实测引用均来自 0127 SMIC 卡与平台 API 实查;"刷屏 15/15"为实测。✔
**顾虑留给评审**:①§6.4 双重计分方案;②空头喂全量 scorecard 的 token/收益权衡;
③情境卡变厚后 regime 枚举 6 标签是否仍够表达(是否需要加"趋势中继/趋势衰竭"类标签);
④聚合行是否会诱导 LLM 把间接截面信号当直接证据(prompt 围栏是否足够)。
**结论:clean for GPT。**
