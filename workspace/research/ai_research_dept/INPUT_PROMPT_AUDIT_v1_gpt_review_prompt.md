# GPT 跨审 prompt — 输入卡/Prompt 专业审计 v1(自包含,直接全文发给 GPT-5.5 Pro)
...
- `momentum_quality` 锚补一句「本股 5d/20d 收益与 RS 相互印证;только放量下的动量才可给 4-5」→
  改为中文:「量价确认过的动量才可给 4-5」。
...
| 两融余额 20d 变动 | `$rzye` 日 bin(approved) | T+1 披露语义:融资余额为 D-1 日值(Tushare margin 每日盘前更新前一日)——**渲染时标注"截至D-1"** ✔ |
