...
| **热点标签** | 焦点词库命中(确定性);新词由 LLM 热词提取(蚂蚁热词智能体模式:先关联词库,关不上才造新词)。**词生命周期(GPT Minor-3):`candidate → approved → deprecated`——LLM 只能铸 candidate,升 approved 须人工/golden set 批准;candidate 词只打标不进热度榜与检索通道**(热点词库最易被 LLM 扩词污染:一个"低空经济"扩出 20 个关联词→全库打标) | 焦点词库=自建,热度分=关联事件量×发酵时长,趋势分=时近性(全确定性计数) |
