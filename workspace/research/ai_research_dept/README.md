...
**实施状态(2026-07-09)**:v1 全量落地(9/10;#10 重放验证待用户读档后定规模)。
v1.5 数据加厚批次 A-F 落地:A 业务构成进基本面卡 · B 研报摘要+report_rc 修正潮事件 ·
C irm_qa 实质问答(quick-LLM 分型)· D 政策三源(npr/货政/联播)无主属行业标注事件 ·
E THS 概念标签+检索通道(泛板块过滤)· F 市场情境简报(确定性卡+LLM 归纳,锚外数字禁令;
进全席 payload 只准校准 confidence/catalyst_timing,prompt v1.1;平台首页情境条)。
v1.5 仍未做(依赖外部件或人工标注):Pass-C 精读卡(PDF 靠 drop-folder)· Pass-R 关系边 ·
叙事记忆卡 · golden set 建库。
平台重构(2026-07-09,用户裁定):新增「个股分析」tab = 决策会话视图(LLM 全部输入四卡原文
+三席/空头/裁判报告,档案 chain_version 标签);全站个股链接指向该页;stock.html 降级为数据明细。
chain_v1.3:news_card 间接节并入概念通道(修复 v1.5-E 半接线——概念通道上线后 industry-only
渲染让间接节静默变空;现存单日档案为旧输入产物,#10 全月重放将用新链版本重建)。
**chain_v2.0(2026-07-09,审计 [INPUT_PROMPT_AUDIT_v1.md](INPUT_PROMPT_AUDIT_v1.md) 实施;
GPT 跨审延后-用户指令,三轮自评通过)**:regime 卡 v0.2 三节化(快照/趋势/持续性,含两融
截至D-1)· fact v0.2(+PS/股息率,+vintage 8 季序列 74k 行)· pv v0.2(F2 修复:两融/北向/
龙虎榜窗口硬截 D-1;+本股收益/换手趋势/净流占比/行业情境行)· 卡渲染 v2(行 ID F/T/N、
格式规范、⚑焦点旗、news 事件龄+星标+类型配额+返回集内聚合+负面缺席声明、披露动态 B 方案)·
prompts v2(证据先行字段序、检查单式职责、结构化证伪 {condition, observable_in})·
空头 v2(全量 scorecard + 证伪回验强度5)。前向前置新增:①b 类数据就绪门(§6.3a)。
