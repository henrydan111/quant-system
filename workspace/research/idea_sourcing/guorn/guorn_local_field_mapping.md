...
| **净资产收益率** (RoeTTM) | TTM归母净利 / 净资产 (**TTM, NOT reported-YTD**) | `(Σ$n_income_attr_p_sq_q[0..3]) / ((($total_hldr_eqy_exc_min_int_q0+_q1)/2))` (lag 0) | ✅ **LEVEL reproducible (2026-06-28 level export):** Spearman **0.96**, med **0.3pp** (TTM归母/avg-eq 0.37pp, TTM总额/end 0.29pp). ⚠ NOT `$roe`/`$roe_waa` (those are **reported cumulative-YTD** → 2.2pp off, Spearman 0.90). Residual ~0.3pp = exact 归母-vs-总额 + end/avg/加权 convention (not pinned; large at neg-equity). **Reusable for RoeQ #38** | 5 |
