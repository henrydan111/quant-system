...
*起草 2026-07-04 · v2 修订 2026-07-04：GPT 5.5 Pro Round-1 = **REVISE**（1 Blocker + 3 Major + 1 Minor，全部接受，无拒绝；GPT 已 fetch 真实 repo + Tushare 文档核实；处置表 §10）· 状态：待 Round-2 re-review*
...
2. **端点就绪 → target_end（强制合同，GPT M1）**：`target_end` = 通过端点就绪合同的最晚 open trading day。合同至少含：trade_cal `is_open==1`；每个必需端点族的 post-vendor-update clock；`daily` 对 D 非空；市场级 daily 端点的期望数/稳定数校验；cyq_perf 的逐股/期望覆盖校验；report_rc 过其文档更新窗**或**显式排除同日完整性要求。任一探针失败 → target_end 回退到全绿的最近 open day。作为新政策 `calendar_end`。
3. **补齐日更未覆盖数据集**到 target_end：namechange 刷新、suspend_d/stock_st_daily 边界、stk_holdertrade、cyq_perf（逐股增量，最长）、index_weights（各月）——复用 `catchup_fundamentals_range.py` 分阶段（断点续跑）。
   ...
   - **⚠ report_rc 必须 fail-closed replay，不是"create_time 增量"**（GPT Blocker B1，已核实 [pit_backend.py:2461-2486](src/data_infra/pit_backend.py#L2461)）：`create_time` 是**输出**可见性字段、非 server-side 查询游标；现有 `contemporaneous = gap_days <= REPORT_RC_BACKFILL_GAP_DAYS(45)` 规则会把**新鲜窗口内 create_time 晚到 >45 天的真实晚到行**误判为 bulk-backfill、锚到 `report_date+2`（早于其真实 create_time = 前视泄漏，代码自己只 WARN）。修正规则：
     - report_rc 按 **report_date replay** 抓取（非假想 create_time 游标）：每次 bump replay `[fresh_holdout_start, target_end]` 全部 report_date + 文档化重叠窗；存 `vendor_create_time` / `raw_fetch_ts` / ingest batch id / provider `as_of_cutoff`。
     - **对 `report_date >= fresh_holdout_start` 的行，禁用 bulk-backfill 回退**：有 create_time → `disclosure = max(report_date, create_time)`；缺 create_time → 隔离或 `disclosure = raw_fetch_ts`，**绝不** `report_date + lag`。任何 post-2026-02-27 行若现有代码会锚得早于 create_time = **build 阻断 PIT 错误，非 warning**。
     - bump 必须失败：replay 发现某 post-2026-02-27 行 `effective_date <= target_end` 但未出现在上次合格 bump 的 raw 快照里，除非该行以不早于其 create_time/raw_fetch_ts 的 effective 加入。
     - 若上述 replay 规则不实现，则 report_rc 移到**每日 22:00 CST 后**的日更作业（滚动 replay 窗 + 同一 fail-closed 锚规则）。
...
- **report_rc fail-closed 锚（B1）**：对 `report_date >= fresh_holdout_start` 禁 bulk-backfill 回退、锚 `max(report_date, create_time)`、缺 create_time 隔离、回锚早于 create_time = build 阻断（改 pit_backend.py report_rc 锚逻辑 + report_date replay 抓取器）
...
**拒绝项：无。** 残余风险（GPT）= report_rc 晚到行处理——已由 B1 fail-closed 规则关闭。
