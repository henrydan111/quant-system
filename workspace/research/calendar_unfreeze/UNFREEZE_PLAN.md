...
| M4 | 缓存世代绑定"传了才校验"≠强制 | **接受**：`record_cache_write`/`assert_cache_reusable` 的两 id **必填非空白**（空白即 raise）；旧空值行失配即拒 = 有意的 legacy 失效路径（月度仪式归档 cache manifest，研究门无静默迁移模式）。**修正（2026-07-02 尖角）**：原实现把"拒绝复用"实现成了"永久拒绝重建"——`assert` 在 `record_cache_write` 之前 raise，轮换后旧 key 永不可自愈（verify07 实测:preload 静默退化为逐日 D.features）。修复：世代失配改抛类型化子类 `ProviderGenerationMismatchError`（仅在 design_hash/stage/window 全部通过后才可能抛出），`qlib_windowed_features`（无缓存工件、必然从 live provider 重算的唯一门）捕获它 → WARNING + 重算 + 追加新世代行 = 自愈；其余调用方照旧向上抛。旧行保留 = 轮换审计痕迹；"接受旧行为有效"的静默路径仍不存在 | cache_manifest + qlib_windowed_features + test_cache_generation_self_heal.py |
