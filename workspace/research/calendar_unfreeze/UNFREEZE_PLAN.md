...
> **执行期注记 3（2026-07-03，首轮冻结段审计 BLOCK + 三类违规全部定性）**：B2 审计按设计拦截发布（23 违规）。诊断闭环：① **indicators 存量截肢（已修复）**——根因是 `refresh_indicator_history` 的 `_swap_live_directory` 无条件整目录换入，我的 2 期解冻补刷把 557k 行存量截成 14k（此前误归因 06-08 重抓，已更正）；从 `indicators_pre_unfreeze_gap_refresh` 归档还原（98 文件/557,708 行，刷新 2 期覆盖其上），部分期段 MERGE 守卫 + 回归测试落地（commit `90999c4`）。② **kline 冻结段字节级干净**（7,486 抽样 bin 零失配，含股本修正字段）。③ **侧车 57 格 = 停牌边界愈合**：8 只冻结边界停牌股复牌后区间端点延伸、重新纳入冻结窗内停牌日（§3.3 语义下 staged 更正确）+ 2 只缺口期退市补记 delist_date——属日历延长的内生修正，进批准例外。**预期残留**：重建 C 复审计时 indicators 族仍有真实修订漂移（06-08 167 列重抓 + update_flag 修订首次入 serving）——按审计条款走"单独批准的 provenance-breaking 迁移"，dry-run 评审时随精确数字签批。
...
| M4 | 缓存世代绑定"传了才校验"≠强制 | **接受**：`record_cache_write`/`assert_cache_reusable` 的两 id **必填非空白**（空白即 raise）；旧空值行失配即拒 = 有意的 legacy 失效路径（月度仪式归档 cache manifest，研究门无静默迁移模式）。**修正（2026-07-02 尖角）**：原实现把"拒绝复用"实现成了"永久拒绝重建"——`assert` 在 `record_cache_write` 之前 raise，轮换后旧 key 永不可自愈（verify07 实测:preload 静默退化为逐日 D.features）。修复：世代失配改抛类型化子类 `ProviderGenerationMismatchError`（仅在 design_hash/stage/window 全部通过后才可能抛出），`qlib_windowed_features`（无缓存工件、必然从 live provider 重算的唯一门）捕获它 → WARNING + 重算 + 追加新世代行 = 自愈；其余调用方照旧向上抛。旧行保留 = 轮换审计痕迹；"接受旧行为有效"的静默路径仍不存在。**GPT 复审 REWORK 折入（同日）**：B1=活跃 `ResearchAccessContext` 以其 `provider_build_id`/`calendar_policy_id` 钉住 run 世代，live 在 run 中途轮换 → 基类硬拒（自愈 catch 吞不掉；D.features 不触达、清单不追加）——正式证据工件绝不跨世代；context 与 live 同世代时 pre-run stale 行仍可自愈（GPT 4c 语义）。M1=最新行改**追加序** `iloc[-1]`（file_lock 下行序 > 秒级 recorded_at；对抗性时钟偏斜测试钉死）。M2=`provider_context.qlib_bound_provider_dir()` 尽力探针进程内 Qlib 绑定，**确证指向非 live provider 即基类硬拒**（staged/归档 provider 不得盖 live id 章；探针不可判定=非证据、放行）。m1=WARNING 附 stage/run_id/step_id。**后果（有意）**：归档 provider 上的 promotion 复现会被 B1/M2 在本门拒绝——与 provider_context 契约一致（重放不走 live helper），重放门属 Phase 3/4。**GPT R2 复审 REVISE 折入（同日；金丝雀通过，B1/M1/m1 关闭）**：R2-M1=读后 TOCTOU 复核——pre-read pin 与 D.features 之间 live 仍可轮换，故 `record_cache_write` 前二次 `live_provider_ids()` 复核，失配即弃读硬拒（沙盒路径同样生效，绝不在可疑世代下写清单行；下次调用自愈/重钉）；R2-M2=探针**不可判定 + 活跃 context → fail-closed**（正式读必须证明进程 Qlib 绑定 live；不可判定的宽容只保留给无 context 沙盒活性）；R2-m1=`run_workspace_pipeline` formal 路径空白 provider id → 构造点 `ValueError`，且置于 `_claim_if_oos` **之前**（畸形调用不烧 spend-on-attempt 封印名额） | cache_manifest + qlib_windowed_features + provider_context + sealed_backtest_runner + test_cache_generation_self_heal.py |
