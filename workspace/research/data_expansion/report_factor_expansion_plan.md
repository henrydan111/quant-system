...
**Round-3 = P1-CODING GO-WITH-NITS — DESIGN SIGNED OFF (2026-06-08).** B1 verified vs live code (no new
operators; the lint-safe inner-`Ref`/`If(x==x,x,0)`/nested-`If` helper is in §3). Nits folded in:
`source_row_hash=sha256(canonical_payload)`, `REPORT_RC_ACTIVE_TTL_OPEN_DAYS=120`,
`EPS_REVISION_EPSILON=1e-4`, `EPS_DISPERSION_FLOOR=0.05` (robust MAD dispersion). **Cleared to start P1
(no-alpha plumbing); first test = `test_report_rc_ledger_preserves_multi_analyst_multi_quarter_rows`.
Still NO-GO on rebuild/screen/promotion.**
...
`qlib_code, normalized_analyst_id, target_fy, quarter, effective_date, report_date, create_time,
source_row_hash` where **`source_row_hash = sha256(canonical_payload_json)`** (canonicalize Unicode,
date/timestamp strings, numeric precision, column order) — persisted as a durable column; NEVER Python
`hash()` or the stripped `_src_*`. (Matches the backend's existing content-hash duplicate-collapse fallback.)
...
**The factor (round-3 VERIFIED lint-safe form — no new operators; `If(x==x,x,0)` null idiom NOT `IsNull`;
INNER-`Ref($field,1)` NOT outer; nested `If` NOT boolean `&`; guarded denom). Generated by a helper:**
```python
def report_rc_eps_rev_breadth(W, min_n=3, min_active=2):
    ...
    z  = lambda x: f"If({x} == {x}, {x}, 0)"            # NaN!=NaN → repo/Qlib null-to-zero idiom
    up_W, dn_W, n_W = f"Sum({z(up)},{W})", f"Sum({z(dn)},{W})", f"Sum({z(rc)},{W})"
    enough = f"({n_W} >= {min_n})"; active = f"({act} >= {min_active})"
    denom = f"If({enough}, {n_W}, 1)"                   # divide-by-zero guard (If evaluates both branches)
    raw   = f"(({up_W} - {dn_W}) / {denom})"
    return f"If({enough}, If({active}, {raw}, np.nan), np.nan)"   # nested If = AND; NaN-mask not clamp
```
Verified vs the live repo: `If`/comparisons/nesting are supported (operators.py L748/L898/L958);
`Sum(If(...))` is the mandated conditional-count idiom (L317); inner-`Ref($field,1)` is lint-safe (the
PIT-safety parser accepts any `Ref` ancestor — test L47-58). **Coding-time smoke check:** confirm a Qlib
expression string accepts the `np.nan` literal via the normal feature path (GPT's recommended smoke test).
`W ∈ {20,60,120}` is a factor hyperparameter. The NaN-mask **silently shrinks the cross-section** → the
screen MUST report eligibility coverage by date/sector/size/coverage bucket. `EPS_REVISION_EPSILON = 1e-4`
(EPS units): `Δ>+ε→up`, `Δ<−ε→dn`, else `same`. `n_active_analysts` is STATE; active until the earliest of
supersession / FY1-roll / `last_visible + REPORT_RC_ACTIVE_TTL_OPEN_DAYS = 120` open days (then a new row =
`coverage_init`, not a revision-vs-stale).
...
defer it (sparse + needs sanity bounds). **`eps_fy1_dispersion` (round-3 robust def):** NaN if
n<3, else `1.4826 * median(|eps_i − median(eps)|) / max(|median(eps)|, EPS_DISPERSION_FLOOR)` with
`EPS_DISPERSION_FLOOR = 0.05` (yuan/share) — robust to one wild forecast, works for negative EPS, floor
stops near-zero-EPS names exploding. (Keep out of P1 unless dispersion is needed; quarantine until registered.) **Defer/quarantine:** `rating_diffusion` (2% coverage,
...
- **P1 — no-alpha PLUMBING slice (first PR).** 1-2 tickers, 1-2 years, quarantined primitive bins only,
  ...
  **P1 MUST include (GPT round-2 mandatory gate):**
  1. explicit `report_rc` `build_ledger` key branch (T5) — **FIRST TEST:
     `test_report_rc_ledger_preserves_multi_analyst_multi_quarter_rows`** on a same-`ts_code`/same-
     `report_date` fixture (2 orgs × 2 authors × 3 quarters + 1 duplicate same-analyst-quarter row with a
     different `create_time`/`source_row_hash`); assert: no collapse to one stock-date row, key includes
     `normalized_analyst_id`+`quarter`/`target_fy`, `effective_date == strictly_next_open(max(report_date,
     create_time/lag))`, deterministic across 2 runs, provider emits only `report_rc__*` quarantine fields.
     Fails on the generic key, passes only after the branch;
