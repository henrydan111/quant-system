# GPT-5.5 Pro re-review #4 (clearing pass) — trading-agents CONTRACTS

复制分隔线以下全部内容发给 GPT-5.5 Pro。CONTRACTS.md + evidence_registry.md **全文已内嵌**(GPT 沙盒无法 fetch GitHub = re-review #2 的 B0 auditability 解法),pinned 到 commit `673937c`。

---

ROLE
You are a senior reviewer for an A-share quantitative research system where RESEARCH VALIDITY outranks code that merely runs (a single lookahead / spent OOS / survivorship filter invalidates the result even if every test passes). This is **RE-REVIEW #4, a CLEARING PASS** of a DESIGN-stage contract spec. Your re-review #3 verdict was **REVISE** with 4 residual findings (2 Major + 2 Minor); ALL were accepted and applied. Confirm closure or surface any remaining blocker. Do not rubber-stamp.

REPO: https://github.com/henrydan111/quant-system  (branch `trading-agents-design`, pinned commit `673937c570e9697d60abfd875dc2d77ce7b2d434`)
The authoritative CONTRACTS.md + evidence_registry.md are EMBEDDED BELOW (your sandbox cannot fetch GitHub — this is the B0 auditability fix from re-review #2). Treat the embedded text as the source of truth.

WHAT WAS APPLIED since your re-review #3 (which ran against pinned `a39e3c1`):
- **R3-Major-1 → C7**: fail-closed NUMERIC overlay caps — the cap config is an immutable pre-registered artifact committed before any Phase 2B/3 eval; `max_name/sector/turnover/active_risk_delta` must be finite, unit-bearing, with hard upper bounds in the strategy registry; missing/null/unbounded/post-hoc-modified/OOS-tuned values FAIL CLOSED; any cap large enough to let the overlay dominate quant-only ranking reclassifies the strategy as `ai_final_decider_shadow` (C6), so the LLM cannot silently become the alpha engine.
- **R3-Major-2 → C14**: status lattice `design_only < test_stub < enforced < evidence_ready` with precise semantics (test_stub = a committed, CI-discovered FAILING/xfail test exists → permits implementation only, NOT build approval; enforced = passes in CI + required for merge; evidence_ready = all phase contracts enforced + experiment/OOS ledger frozen). Reconciled the earlier "passing tests before build" wording: design→build needs ≥test_stub; alpha evidence needs enforced/evidence_ready; a `.md` existing is NOT test_stub.
- **R3-Minor-1 → evidence_registry**: every `🔎` row now carries `Support=unverified_pending` + `Design implication=caution_only_until_refetched`; only `✅me` rows may use supported/partial/contradicted.
- **R3-Minor-2 → C4**: C4 can be unparked ONLY via a signed `IPO_UNPARK_REQUEST.md` that flips the C14 status `parked→test_stub`, names owner modules, and lists the applicant-ledger source plan; README/ROADMAP drift cannot reopen it.

HONEST NOTE: the required pytest files per finding are **test_stub-stage obligations**; the design stage is all `design_only` — **no tests are written yet** (NOT fabricated as green). The contracts are spec text awaiting implementation.

===== EMBEDDED AUTHORITATIVE DOCS (pinned 673937c) follow =====


--- CONTRACTS.md (pinned 673937c) ---
...
--- evidence_registry.md (pinned 673937c) ---
...
QUANTITATIVE-RESEARCH PRINCIPLES (a violation = Blocker): 1 PIT / no-lookahead (incl. the decision-LLM's parametric memory of post-decision events); 2 OOS sealed, single-shot / spend-on-attempt; 3 survivorship (delisted+suspended stay in the universe); 4 factor selection by MARGINAL orthogonal contribution, not standalone ICIR; 5 execution/cost realism (event-driven total-return, 1x, T+1 / limits / suspension / corp-actions); 6 NO leverage (gross <= 1x); 7 NO hedge words (named source or marked unverified); 8 four-layer pipeline (factor -> universe mask -> signal rank -> tradability ONLY in execution); 9 multiple testing (count effective trials; DSR/PSR/FDR/PBO).

CLEARING-PASS QUESTIONS
1. Are R3-Major-1 (C7) and R3-Major-2 (C14) now fully closed as written — fail-closed and machine-checkable? Quote any residual gap with an exact replacement.
2. Is C14's status-lattice semantics now precise enough that phase-advancement and alpha-evidence gates are machine-checkable? (This was your re-review #3 "single most important residual risk.")
3. Are R3-Minor-1 (registry visibility-row labels) and R3-Minor-2 (C4 signed-unpark gate) closed?
4. Any NEW issue introduced by the R3 edits — internal contradiction, ambiguity, or principle violation?
5. Is the "design stage = all design_only, tests not yet written" stance acceptable for a DESIGN-stage clearing pass, given C14 itself forbids any alpha evidence until contracts are enforced/evidence_ready?

OUTPUT FORMAT
- Issues ranked Blocker / Major / Minor — quote the offending clause + give an exact replacement; map each Blocker to the violated principle/invariant. If a category is empty, say so explicitly.
