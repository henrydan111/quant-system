...
binds to [CONTRACTS.md](CONTRACTS.md) C1/C2/C8/C12/C15/C16;**§10 re-review #7 = REVISE(5B+3M+3m,全应用)→ re-review #8 = SHIP(2026-07-05,0 Blocker / 0 Major / 3 Minor 加固,已应用:labeler_conflict_screen / semantic_family_key / cluster_algo_version 替换规则)**;all `design_only`(C14)。
...
- **cluster_algo_version 替换规则(R8-m3)**:对旧历史重新聚类产生的新 `cluster_algo_version`/`cluster_run_id`
  → 新 `refinery_config_version` + **新 panel namespace** + 新 CandidateID;**不得覆盖/替换/"修正"已入闸
  或已花费 OOS 的历史面板**;每个 CandidateID 的 active cluster version 须在收益访问前 **registry-pinned**。
...
- **labeler_conflict_screen(R8-m1)**:标注者**不得**标注其在事件窗口及后续收益窗口内主动交易、深度研究、
  写过复盘、或明确记得市场结局的发行人/行业/事件。每个 packet 记录 `labeler_conflict_attestation =
  {none, issuer_memory, sector_memory, traded, researched, outcome_recognized}`;**非 `none` 一律
  `hindsight_risk=true`,不得进校准集**。Golden set 维持预注册的 **famous/non-famous 分层覆盖**,且不得
  因 famous-event 标签质量改善而改变收益闸门结论。
...
- **提纯配置入闸节流(R7-B3——"全计入有效试验"不够,须节流)**:一个 `refinery_calibration_cycle` 内,
  ...
  周期 + 回滚计为新探索变体。**
- **semantic_family_key(R8-m2)**:`signal_family` 是**语义键而非名称键**。重命名、近等价定义、同一原始
  字段的单调变换、同一簇字段的窗口/阈值微调,**必须归入同一 `family_group_id`**——除非在任何收益访问前
  预注册为独立科学假设并计为完整 CandidateID。**收益访问后不得拆分/合并/重命名/重标 family 来规避
  one-winner throttle 或 effective-trials 计数。**
...
- **(R8 新增)** `tests/text/test_labeler_conflict_attestation_required.py`(attestation≠none → 排除校准集)
  · `tests/text/test_semantic_family_group_id_alias_guard.py`(改名/单调变换归同 family_group_id)
  · `tests/text/test_cluster_algo_version_new_namespace.py`(重聚类=新 namespace,不覆盖已花费面板)
