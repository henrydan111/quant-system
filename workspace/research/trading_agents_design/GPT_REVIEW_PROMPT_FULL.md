# GPT-5.5 Pro re-review #3 — full-text-embedded closure audit (resolves B0)

复制下面分隔线以下的**全部内容**发给 GPT-5.5 Pro。本版**内嵌了 CONTRACTS.md + evidence_registry.md 全文**(re-review #2 唯一 Blocker B0 = GPT 沙盒无法 fetch GitHub)→ 无需联网即可逐条审 C1–C14。

---

ROLE
You are a senior reviewer for an A-share quantitative research system where RESEARCH VALIDITY outranks code that merely runs. This is a CLOSURE AUDIT (re-review #3). The binding contracts (CONTRACTS.md C1–C14) and evidence_registry.md are EMBEDDED BELOW IN FULL, pinned to commit a39e3c135d4e7fa33e4df56416147cba744f0861 — you do NOT need to fetch anything. A single lookahead, a spent/contaminated OOS, or a survivorship filter invalidates the research even if code passes. Be skeptical; do not rubber-stamp.

B0 RESOLUTION — re-review #2's sole Blocker was an AUDITABILITY failure: your sandbox could not resolve raw.githubusercontent.com, so you could not certify the binding contract text (a summary/closure-map cannot substitute). RESOLVED: the full CONTRACTS.md + evidence_registry.md are pasted verbatim below, pinned to the commit SHA above.

REPO (optional cross-check only — embedded text is authoritative)
https://github.com/henrydan111/quant-system  (branch trading-agents-design, commit a39e3c135d4e7fa33e4df56416147cba744f0861)
Raw form: https://raw.githubusercontent.com/henrydan111/quant-system/a39e3c135d4e7fa33e4df56416147cba744f0861/<path>

CLOSURE MAP (prior finding → contract clause): B1→C1, B2→C2, B3→C3, B4→C4(parked), B5→INTEGRATION_RDAGENT F3-unpark + C2, B6→C5; M1→C14, M2→C4-scope, M3→C1-byte-replay, M4→evidence_registry-schema, M5→C1-adapter, M6→C7, M7→C9; m1→doc status banners, m2→ROADMAP smoke-test, m3→C6-reporting.

CONTEXT for the contract (fetch optional; closures are all in the embedded text):
- CLAUDE.md — §3 invariants, PIT §3.2, sealed-OOS §3.4/§3.5, §7 integrity, §7.10 no-hedge, §7.11 no-leverage:
  https://raw.githubusercontent.com/henrydan111/quant-system/a39e3c135d4e7fa33e4df56416147cba744f0861/CLAUDE.md
- ROADMAP.md (plan of record) + BLUEPRINT / INSTITUTIONAL_WORKFLOW / INTEGRATION_RDAGENT[PARKED] / HARNESS_FORWARD_AI_TRADER:
  https://raw.githubusercontent.com/henrydan111/quant-system/a39e3c135d4e7fa33e4df56416147cba744f0861/workspace/research/trading_agents_design/ROADMAP.md  (siblings in same dir)

QUANTITATIVE-RESEARCH PRINCIPLES — a violation is a Blocker: (1) PIT/no-lookahead incl. the LLM's parametric memory of post-t events; (2) OOS sealed / single-shot; (3) survivorship (incl. delisted/suspended); (4) factor-eval by MARGINAL orthogonal contribution; (5) execution/cost realism — event-driven total-return 1× with realistic A-share costs is the deployable number; (6) NO leverage; (7) NO hedge words; (8) four-layer pipeline (tradability ONLY in execution); (9) multiple testing — effective trials, DSR/PSR/FDR/PBO.

REVIEW INSTRUCTION (re-review #3)
1. For EACH C1–C14: does the clause AS WRITTEN below fully close its mapped blocker with FAIL-CLOSED (not MAY/SHOULD) language? Quote the clause; mark CLOSED / PARTIAL / OPEN with a one-line reason.
2. Any NEW blocker the contracts themselves introduce (over-broad ban, internal contradiction, unenforceable gate, a contract that conflicts with a CLAUDE.md §3 invariant)?
3. Is evidence_registry's verified/unverified separation (M4 schema) sufficient to bar unverified papers from becoming contract language?

OUTPUT FORMAT
- Per-contract closure table (C1–C14: CLOSED / PARTIAL / OPEN + one-line reason).
- Any residual/new Issues ranked Blocker / Major / Minor, each quoting the offending clause + an exact fix, mapped to the principle/invariant it violates.
- Final line: SHIP / REVISE / REWORK + the single most important residual risk.

===== EMBEDDED: CONTRACTS.md (authoritative, pinned a39e3c1) =====
...
| Memorized historical financial data is a lookahead channel that inflates apparent accuracy and collapses OOS | MemGuard-Alpha | [2603.26797](https://arxiv.org/abs/2603.26797) | 🔎 GPT-surfaced (not fetched by me) | supported | C2; reinforces cutoff-overlap risk |
...
| FinMem: Profile + layered Memory + Decision; memory-augmented agent | FinMem | [2311.13743](https://arxiv.org/abs/2311.13743) | 🔎 search-level (html fetch returned wrong content) | supported (architecture) | Phase 2B memory module |
| Offline LLM-trading eval structurally leaks; stronger LLM ≠ better trading (LMArena corr ≈ 0) | LiveTradeBench | [2511.03628](https://arxiv.org/pdf/2511.03628) | 🔎 deep-research search-surfaced (not re-fetched) | supported | C5 forward eval |
| Standardized look-ahead-bias benchmark; "Scaling Paradox" (bigger LLM worse OOS) | Look-Ahead-Bench | [2601.13770](https://arxiv.org/pdf/2601.13770) | 🔎 deep-research search-surfaced (not re-fetched) | supported | C2 cutoff risk |
