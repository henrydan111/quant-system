...
- **Pre-open no-lookahead (2026-06-22)**: the `before_market_open` strategy hook gets a context whose `day_data`/`day_data_indexed` are EMPTY — same-day OHLCV is withheld so a strategy cannot read today's close/high/low pre-open (its contract is "prev_day_data only"). The engine keeps the real `day_indexed` LOCAL for execution gating; `on_bar` (EOD) gets the full frame. *Enforced:* `tests/backtest_engine/test_pre_open_isolation.py`.
