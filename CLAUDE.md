...
- **Limit detection**: `Exchange.resolve_limit_prices()` prefers Tushare published `$up_limit`/`$down_limit` (bare-name day bins), falls back to `compute_limit_prices(pre_close, band)` only when absent/NaN. The published field carries fen-rounding, ex-rights adjustment, the pre-2023 +44%/−36% IPO-first-day rule, and post-2023 no-limit windows. `$up_limit`/`$down_limit` are in `ENGINE_REQUIRED_FIELDS`; `stk_limit` is `approved` (these are session-open-knowable execution fields, no `Ref` lag). **Fill-price-aware gate (2026-06-22)**: `is_limit_up`/`is_limit_down`/`can_buy`/`can_sell` take `price_field` (default `raw_close` = legacy); `_execute_orders` passes the FILL column, so an OPEN fill gates on `raw_open` — a name LOCKED-up at the open (一字/调仓时涨停) is unbuyable, but one that merely CLOSES limit-up was buyable at 09:35. Close-based gating on open fills silently under-bought bull-market winners in every open-fill backtest (surfaced by the 果仁 sm_纯市值01 parity). The daily-AVERAGE fill mode (`jq_daily_avg`) instead gates on `limit_gate='all_day_lock'` (`is_all_day_limit_up/down`: high==low==limit = 一字), since a synthetic avg price is not a tradability state. **No-limit-day buy bypass (2026-06-22, GPT cross-review Major-2)**: `can_buy`'s limit-up bypass uses `is_true_no_limit_day(code, date, row)`, NOT the old board+days-since-listing `is_ipo_period`. A day is no-limit ONLY on a published no-limit SENTINEL (`up_limit≥99999` ∧ `down_limit≤0.01` — incl. the BSE 99999.99 form) OR a confirmed *listing-date-regime* IPO window (post-2020-08-24 ChiNext / STAR: first 5 trading days; post-2023-04-10 全面注册制 main board: first 5 days; BSE: listing day only). It does NOT treat the OLD main-board / pre-2020 ChiNext **+44%/−36% first day** as no-limit (that cap is a REAL published limit) — `is_ipo_period` wrongly did, so a name locked at its +44% first-day limit was "buyable". `is_ipo_period` is retained as the nominal-window helper but is DEAD in the gate; do not re-wire it. *Enforced:* `test_exchange_limits.py::ResolveLimitPricesTests` (incl. the open-vs-close pins) + `::IsTrueNoLimitDayTests`, `test_field_registry.py::...stk_limit_bare_fields_approved_for_formal`.
