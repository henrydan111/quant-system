...
- [src/alpha_research/](src/alpha_research/) — factor library (177-factor catalog: 153 base + 4 industry-relative + 20 Layer-2 composites), factor evaluation toolkit, theme strategy framework, model zoo, MLflow tracker, factor/candidate registries
...
- **Live registry state (2026-06-02)**: 87 `candidate` + 84 `draft` + 6 `approved`. The 6 approved are the Round-6 sealed-OOS winners (`liq_zero_ret_days_10d`, `rev_turnover_spike_5d`, the 3 `grow_*_yoy_accel_q`, `qual_piotroski_fscore_9pt`). The phase-6/7 candidates are an `oos_informed_backfill` (2021-2026 is BURNED for them; the candidate→approved gate must use a genuinely-sealed window). Factor `approved` ≠ tradable-strategy validated — strategy deployment is a separate downstream gate. Provenance: the JSONs under [workspace/research/factor_expansion/](workspace/research/factor_expansion/).
- **Catalog count = 177**: 153 base + 4 industry-relative + 20 Layer-2 composites. The live breakdown source of truth is `test_pr9_validation_field_gate.py::TestFormalFactorCompatibility`.
