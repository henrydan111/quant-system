...
- **Bare share-capital bins are EFFECTIVE-DATE anchored, legacy-unit asymmetric (2026-07-01)**: `$total_share` (股 — raw daily 万股 ×1e4) / `$float_share` / `$free_share` (万股 verbatim) are materialized from the raw `market/daily` columns by `_materialize_share_capital_daily` (`force=True` — runs in every build mode regardless of `datasets`/`field_filter`; ffill across suspensions, no back-fill). The balancesheet snapshot compat alias must NEVER clobber these bare names (it did — the 果仁-parity bug: 1-2 months report lag, contradicted `$total_mv`); the report-period series remains `$total_share_q0..qN` and is CORRECTLY report-anchored — do not "fix" it. An in-place correction of APPROVED field values requires: provider-id rotation (+ patch lineage in `data/qlib_data/metadata/provider_patches.jsonl` — schema v1 forbids inline patch keys), an `approved_field_value_correction` entry in `field_approval_log.jsonl`, and a `data_correction_replay` for any approved factor consuming the field (precedent: `qual_piotroski_fscore_9pt`, PASS — see project_state 2026-07-01d). *Enforced:* [tests/data_infra/test_share_capital_daily.py](tests/data_infra/test_share_capital_daily.py).
...
- **Limit detection**: `Exchange.resolve_limit_prices()` prefers Tushare published `$up_limit`/`$down_limit` (bare-name day bins), falls back to `compute_limit_prices(pre_close, band)` only when absent/NaN. The published field carries fen-rounding, ex-rights adjustment, the pre-2023 +44%/−36% IPO-first-day rule, and post-2023 no-limit windows. `$up_limit`/`$down_limit` are in `ENGINE_REQUIRED_FIELDS`; `stk_limit` is `approved` (these are session-open-knowable execution fields, no `Ref` lag). **Fill-price-aware gate (2026-06-22)**: `is_limit_up`/`is_limit_down`/`can_buy`/`can_sell` take `price_field` (default `raw_close` = legacy); `_execute_orders` passes the FILL column, so an OPEN fill gates on `raw_open` — a name LOCKED-up at the open (一字/调仓时涨停) is unbuyable, but one that merely CLOSES limit-up was buyable at 09:35. Close-based gating on open fills silently under-bought bull-market winners in every open-fill backtest (surfaced by the 果仁 sm_纯市值01 parity). The daily-AVERAGE fill mode (`jq_daily_avg`) instead gates on `limit_gate='all_day_lock'` (`is_all_day_limit_up/down`: high==low==limit = 一字), since a synthetic avg price is not a tradability state. **No-limit-day bypass (2026-06-22, GPT cross-review Major-2; R2 = can_sell symmetry + BSE/fail-closed hardening)**: BOTH `can_buy` (limit-up) and `can_sell` (limit-down) bypass the limit block via `is_true_no_limit_day(code, date, row)`, NOT the old board+days-since-listing `is_ipo_period`. A day is no-limit ONLY on a published no-limit SENTINEL (`up_limit≥99999` ∧ `down_limit≤0.01` — incl. the BSE 99999.99 form) OR a confirmed *listing-date-regime* IPO window (post-2020-08-24 ChiNext / STAR: first 5 trading days; post-2023-04-10 全面注册制 main board: first 5 days; BSE: listing day only). It does NOT treat the OLD main-board / pre-2020 ChiNext **+44%/−36% first day** as no-limit (that cap is a REAL published limit) — `is_ipo_period` wrongly did, so a name locked at its +44%/−36% first-day limit was "buyable"/"sellable". Out-of-universe codes (B-share / fund / unknown board) **fail CLOSED** (regime branch returns False; only a published sentinel can mark them no-limit). Board classification is the shared `_is_bse_code` (.BJ suffix OR 43/83/87/88/92 incl. 920) + `_MAIN_BOARD_PREFIXES` allow-list, used by `is_true_no_limit_day` AND `get_limit_pct` (so a BSE coverage-hole name resolves the ±30% band, not ±10%). `is_ipo_period` is retained as the nominal-window helper but is DEAD in the gate; do not re-wire it. *Enforced:* `test_exchange_limits.py::ResolveLimitPricesTests` (incl. the open-vs-close pins) + `::IsTrueNoLimitDayTests` + `::LimitPriceTests` (BSE 88/920, B-share), `test_field_registry.py::...stk_limit_bare_fields_approved_for_formal`.
- **Pre-open no-lookahead (2026-06-22)**: the `before_market_open` (and `initialize`) strategy context withholds same-day data on THREE channels, not just the frames: `day_data`/`day_data_indexed` are EMPTY; `context.feeder` is a `PhaseBoundFeeder` (`get_features` capped at `end_time <= prev trading day`); `context.exchange` is a `StrategyExchangeView` (forwards public tradability methods, refuses `_feeder`/`_inner`/private access). Both use `__getattribute__`+`__slots__` so the raw object can't escape via `_inner` (a bare `__getattr__` would NOT close that). The engine keeps the real `day_indexed`/feeder/exchange LOCAL for execution gating and restores them ONLY for `on_bar` (EOD). Do NOT "simplify" the wrappers back to empty-frames-only. *Enforced:* `tests/backtest_engine/test_pre_open_isolation.py` (attacks `get_features(today)`, `_inner`, `_feeder`, `__dict__`). See `pre_open_guard.py`.
...
- **Cache provider-generation binding + rotation self-heal (M4, 2026-07-02)**: every cache-manifest row records `provider_build_id`+`calendar_policy_id` (REQUIRED non-blank); `assert_cache_reusable` refuses a latest row from another generation via the typed subclass `ProviderGenerationMismatchError` — raised ONLY after design_hash/stage/window pass, so catching it can't mask those. The ONE recompute-only door [qlib_windowed_features](src/research_orchestrator/qlib_windowed_features.py) self-heals on rotation: WARNING → recompute from the live provider → append a fresh row under the live generation (stale row kept as the rotation audit trail); every other caller stays fail-closed. Formal reads: an active `ResearchAccessContext` PINS the run generation (live≠context → base-class hard fail, checked pre-read AND re-checked post-read before `record_cache_write` — TOCTOU); an INCONCLUSIVE in-process Qlib-binding probe (`provider_context.qlib_bound_provider_dir()`) under an active context fails closed, a POSITIVE non-live binding fails closed for everyone (no-context sandbox stays fail-open on inconclusive only); `run_workspace_pipeline` with blank provider ids raises BEFORE the spend-on-attempt seal claim. "Latest" manifest row = APPEND order under `file_lock`, never `recorded_at` sort. Known limit (accepted): id-based guards see only rotations that rewrite `provider_build.json` — a byte-swap without a manifest rewrite is a provider-attestation violation upstream, invisible here. *Enforced:* [test_cache_generation_self_heal.py](tests/research_orchestrator/test_cache_generation_self_heal.py), `test_assert_cache_reusable_permissive.py::TestGenerationBindingFailClosed`. GPT §10: 3 rounds REWORK→REVISE→**SHIP** (2026-07-02).
