...
def _complete_provenance() -> ArtifactProvenance:
    ...
    + execution_profile_hash. PR 8 fix #5 added a hash-mismatch check in
    the release gate, so the helper now uses the live profile_hash (not a
    fake "0" * 64) so the artifact is actually reproducible. Tests that
