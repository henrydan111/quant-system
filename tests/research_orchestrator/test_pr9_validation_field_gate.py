...
class TestFormalFactorCompatibility:
    ...
    KNOWN_NON_FORMAL_FACTORS: dict[str, dict[str, Any]] = {
        # As of the 2026-06-04 gated reviews, the ONLY still-non-approved
        # registered dataset is margin_detail_repayment (held repayment fields
        # $rzche/$rqchl - BSE .BJ negatives). moneyflow, hk_hold, margin_detail
        # (balance/buy fields), top_list, top_inst, block_trade, cyq_perf, and
        # stk_holdertrade were all promoted to approved in those reviews, so
        # their dependent factors are now formal-eligible and were removed here.
        "margin_net_buy_20d": dict(
            reason="depends on margin_detail_repayment quarantine ($rzche held: BSE .BJ negatives)",
