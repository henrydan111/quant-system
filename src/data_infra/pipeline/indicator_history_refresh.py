...
# Full fina_indicator output field set (Tushare doc/2 doc_id=79, 167 fields). Requested
# explicitly so the refresh stores ALL indicators, not just Tushare's default-显示=Y subset
# (the 2026-06-08 field-parity audit found 58 non-default indicators were being dropped
# because no `fields=` was passed). Per-period all-null columns are still dropped at fetch.
FINA_INDICATOR_ALL_FIELDS = (
    ...
    "gc_of_gr,op_of_gr,ebit_of_gr,roe,roe_waa,roe_dt,roa,npta,roic,roe_yearly,roa2_yearly,"
    ...
    "q_op_yoy,q_op_qoq,q_profit_yoy,q_profit_qoq,q_netprofit_yoy,q_netprofit_qoq,equity_yoy,"
    "rd_exp,update_flag"
)
...
class IndicatorVipHistoryRefresher:
    ...
    def fetch_period(self, period: str) -> pd.DataFrame:
        df = self.fetcher.fetch_fina_indicator_vip(period=period, fields=FINA_INDICATOR_ALL_FIELDS)
