...
def _claim_holdout_access_if_needed(context: StepExecutionContext) -> None:
    ...
    # v1.4 A8 (implementation-review rounds 1-2, 2026-07-03): this chokepoint claims by the
    ...
    # which claims through the skill's own seal path) — refuse it BEFORE the seal is spent.
    # The SHARED guard also protects the OTHER legacy claim site,
    # SealedBacktestRunner._claim_if_oos (round-2 Blocker 1). Already-burned windows
    # (oos_end <= 2026-02-27) still pass: the required PR3 dry-run pilot runs on a burned
    # window through these same paths.
    ...
    assert_v14_a8_no_legacy_virgin_oos_claim(
        stage="oos_test",
        time_split=hypothesis.time_split.to_dict(),
        caller="orchestrator OOS seal-claim chokepoint (_claim_holdout_access_if_needed)",
    )
