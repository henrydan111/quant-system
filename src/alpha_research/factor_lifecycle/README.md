...
| **approved (legacy)** | ⚠ **No longer minted (v1.4), and the live registry has 0.** The 7 pre-v1.4 rows were REVERTED to `candidate` on 2026-07-04 (user directive; evidence + status_history + provenance JSONs preserved). Promotion moved to the **book level**: one holdout seal per sealed `DeploymentFrozenPlan`/`StrategyCandidate` (key = derived `book_seal_key`), published into `strategy_registry`. | Any FUTURE legacy row would resolve as `formal` (subject to `approval_validity`; re-affirmation only via `revalidate_legacy_approved`); the `legacy_factor_approval_override` machinery remains. |
