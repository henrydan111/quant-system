...
果仁 = the trusted external benchmark; the local system is **under test**. Goal: reproduce 果仁 books, verify local factors/data, and approximate 果仁's backtest returns. **NON-FORMAL** diagnostic work: it reads the published PROVIDER (`D.features`, already PIT-aligned at build, like the harnesses) and honors CLAUDE.md §3 — never read raw `data/pit_ledger/*`, never hand-roll PIT alignment; FORMAL factor work routes through the sanctioned wrappers + `get_factor_catalog()`.
...
| "The gap is execution / costs / limit-ups." (stated without testing) | **Decompose by REPLAY first**: feed 果仁's exact held names (+ closest weights/cost/fill) through the engine. replay ≈ 果仁 ⇒ selection is the DOMINANT residual (names-only ≠ full isolation; weights/cost/fill unverified); replay gap ⇒ execution. Never attribute without it. |
| "Local ≠ 果仁, so our data is wrong." | Read the residual **lag → unit → 复权 → calendar/window-membership → vendor → bug, in that order**. 果仁's vendor (朝阳永续) / 复权 / calendar legitimately differ; localize before claiming a bug. |
