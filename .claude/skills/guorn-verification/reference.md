...
| **自定义指标 semantics — 果仁's OFFICIAL operator + financial-formula ground truth** (REQUIRED before writing a local expr for any 果仁 custom indicator) | [Knowledge/果仁帮助文档/](Knowledge/果仁帮助文档/) — `自定义函数列表.txt` (every operator `Ref`/`BarRef`/`Delta`/`MA`/`EMA`/`RefQ`/`TTM`/`hSum`/`Level`/`KLast`… + the load-bearing gotchas: `Ref` counts 停牌 days / `BarRef` skips them, negatives written `0-1`, no `%`, indicator-naming rules), `自定义指标实例列表.txt` (38 worked custom-indicator formulas), `财务指标公式.txt` + `财务指标函数的使用实例.txt` (exact 果仁 ratio defs — 市盈率/ROE/RefQ- & TTM-growth), plus `果仁选股指标详解.txt` / `果仁筛选与排名功能.txt` / `策略评估指标详解.txt` / `果仁市场择时功能.txt` / FAQ. This is 果仁-OFFICIAL; the derived files below are decoded/AI-assisted — cross-check them against these. |
| **raw 果仁 indicator formulas** (workspace-derived / decoded — cross-check vs the OFFICIAL docs above) | `indicator_reference_auto.md`, `guorn_aichat_indicator_defs.md`, `内联公式85条拆解.md` (same dir) |
| **methodology + lessons + current state** | memory `project_guorn_parity` + `project_state.md` |

**Web-export operational (validated 2026-07-01) — read BEFORE driving the UI:**
- 果仁 exports auto-download to **`E:\量化系统\Knowledge\果仁验证因子\`** (the browser's preset download dir — **NOT `~/Downloads`; don't hunt there**) as a **garbled `<hash>.xlsx`** → confirm with `ls -t Knowledge/果仁验证因子/*.xlsx` and **rename immediately** to `果仁_{YYYYMMDD}_{universe}_{条件}.xlsx`.
- **One factor per export** (exactly ONE 排名条件) so each top-K is clean — do NOT batch factors into one export.
- 导出 is **async + flaky**: the click fires `POST /stock/export/screen` → `GET /file/<hash>.xlsx`, and that GET often **503s (file not ready) → briefly 200 → 404** (short serve window). The browser's own GET frequently loses the race, BUT the file still lands in the folder — **just re-click 导出 and re-check `ls -t` the folder**. A valid file is **~300KB+**; a 69-byte file is the 404-HTML stub (discard). Don't waste time blob-fetching `/file/<hash>` in JS — read the saved `.xlsx` from disk.
- Native `<select>` (ST/科创) + Chinese-name indicator search can't be typed → **JS-set** the input `.value` + dispatch `input`, then JS-click the suggestion's **parent row** (not the inner `<span>`). Fire 导出 with a full `mousedown→mouseup→click` MouseEvent sequence (a bare `.click()` may not trigger it). Coordinate clicks drift (viewport≠screenshot scale) — prefer JS-clicks by text.
...
3. **Factors** — map each ranking/filter indicator to its local expr via `guorn_local_field_mapping.md` (use the validated row; if missing, validate it 字段级 FIRST). Single-quarter = `$<f>_sq_q0`; TTM = Σ`_sq_q0..q3`. **If the indicator is a 自定义指标 (user-composed), resolve every operator + financial ratio from 果仁's OFFICIAL help docs ([Knowledge/果仁帮助文档/](Knowledge/果仁帮助文档/)) BEFORE writing the local expr** — 果仁's functions have exact, sometimes non-obvious semantics (`Ref` counts 停牌 days but `BarRef` skips them; `RefQ($f,4)` = same quarter one year ago; `TTM($f,0)` = latest 4 quarters; TTM ratios divide by `AvgQ(...,4,1)`; `Level`/`KLast` are bar-resample/period ops). Guessing 果仁's operator behavior is exactly the rationalization this skill forbids — a mis-decoded operator silently breaks both the value-parity and the top-K gate.
