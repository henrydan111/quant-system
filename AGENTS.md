...
- **Live registry state (2026-06-08)**: `factor_master.parquet` holds 243 rows — 90 `candidate` + 147 `draft` + 6 `approved`; current set = 182 (86 draft + 90 candidate + 6 approved). `qual_gross_profitability` (OSAP-sourced) added + promoted `draft→candidate` (IS-only `factor_lifecycle`, heldout RankICIR 0.138) on 2026-06-08, then its sealed-OOS approval test FAILED (deterministic reproduction: OOS RankICIR −0.12 sign-flipped vs IS +0.14, LS Sharpe −0.66 < bar 1.0) → stays `candidate`, NOT approved; 2021-2026 OOS observed/spent (don't re-test as fresh). +2 OSAP Tier-1 drafts (`qual_cash_to_assets`/`qual_rd_to_assets`, asset-composition/R&D, +0.019/+0.017 increment) added 2026-06-08, then **repointed to registered PIT fields** (`$money_cap_q0`/`$total_assets_q0` already approved + `$rd_exp_sq_q0` promoted into `income`, parity 0-mismatch) → now **formal-eligible drafts (v2)**, removed from `KNOWN_NON_FORMAL_FACTORS`; `rd` is sub-universe (cov ~0.31). **`report_rc` integrated+approved 2026-06-09** (4 `$report_rc__*` primitives) → 2 **candidates** `earn_eps_diffusion_60`/`earn_eps_diffusion_120` (analyst EPS-revision breadth) — first new-dimension idea-sourced factor (compliant size-neut IS ICIR ~0.55/0.46, SURVIVES orthogonalization to ROE/growth); PASSED the IS `factor_lifecycle` gate 2026-06-09 (heldout RankICIR 0.42/0.34, sign-consistency 1.00/0.86 → draft→candidate; NOT oos_informed_backfill, OOS genuinely unburned). ⚠ candidate→approved sealed-OOS HARD-GATED behind the 2026-06-15 breadth canary (OOS spend forbidden until pass). The 6 approved are the Round-6 sealed-OOS winners. The phase-6/7 candidates are an `oos_informed_backfill` (2021-2026 is BURNED for them; candidate→approved must use a genuinely-sealed window). Factor `approved` ≠ tradable-strategy validated. Provenance: JSONs under `workspace/research/factor_expansion/`.
