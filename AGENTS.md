...
- **Live registry state (2026-06-05)**: `factor_master.parquet` holds 236 rows — 87 `candidate` + 143 `draft` + 6 `approved` (draft pool exceeds the ~84 catalog-draft factors due to new-data factor-expansion `draft` entries; candidate/approved are catalog-aligned). The 6 approved are the Round-6 sealed-OOS winners. The phase-6/7 candidates are an `oos_informed_backfill` (2021-2026 is BURNED for them; candidate→approved must use a genuinely-sealed window). Factor `approved` ≠ tradable-strategy validated. Provenance: JSONs under `workspace/research/factor_expansion/`.
